...
// Constants (move these to a separate config file for better organization)
const TOAST_CONFIG = {
  position: "top-right",
  autoClose: 5000,
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    if (address && solanaAddress) {
      ...
      // If user is loaded but addresses are not present
      showErrorToast("Please connect your wallet to view data.");
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    ...
    if (address) {
      const db = getFirestore(app);
      const workersRef = collection(db, "businesses", address, "workers");
    ...
    filtered = filtered.filter(
      (transaction) =>
        ...
        (transaction.transactionId &&
          transaction.transactionid?.toLowerCase().includes(lowerSearch)) || // Corrected
