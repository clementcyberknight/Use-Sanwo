...
export default function PayrollPage() {
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      if (address) {
        ...
        const companyDocRef = doc(db, "businesses", address); // Assuming company ID is the document ID
        try {
          ...
          if (docSnap.exists()) {
            ...
            unsubscribe = onSnapshot(
              ...
              (snapshot) => {
                ...
                updateDoc(doc(db, "businesses", address), {
                  totalPayroll: newTotalPayroll,
                  employeeCount: newEmployeeCount,
                }).catch((error) => {
                  console.error(
  ...
  }, [userContext.user, address, router]);
  ...
  const handleAddWorker = async () => {
    ...
    try {
      ...
      await setDoc(
        doc(db, "businesses", address, "workers", worker_id),
        workerData
      );

      // Update business metrics
      await updateBusinessMetrics(db, address, salaryNum);
  ...
  const processBulkUpload = async (data) => {
    ...
    try {
      ...
      await updateBusinessMetrics(db, address, totalAddedSalary);
      ...
      <div className="border-b mb-6">
        <div className="flex gap-8">
          {["WORKER LIST", "INVITE WORKER", "BULK WORKER INVITE"].map((tab) => (
            <button
              key={tab}
              className={`pb-2 transition-colors duration-200 ${activeTab === tab
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {filteredWorkers.map((worker) => (
                      <tr key={worker.worker_id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`text-sm ${worker.role === "Developer"
                              ? "text-red-600"
                              : worker.role === "Manager"
                                ? "text-green-600"
                                : "text-black"
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${worker.status === "invited"
          ...
          <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
            ...
            <div className="mt-4">
              ...
              <div className="relative">
                ...
                />
                <div
                  className={`w-full p-6 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 transition-colors ${uploadedFileName ? "border-green-500" : ""
                    }`}
                >
